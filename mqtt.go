...
func payloadToCommand(payload string) (string, error) {
	switch payload {
	case "MAKE_SOURCE_INACTIVE":
		return "is", nil
	case "MAKE_SOURCE_ACTIVE":
		return "as", nil
	// https://www.cec-o-matic.com/
	case "SOURCE_HDMI_1": // TODO
		return "spl 1", nil
	case "SOURCE_HDMI_2": // TODO
		return "spl 1", nil
	case "SOURCE_HDMI_3":
		return "tx 4f:82:30:00", nil
	case "SOURCE_HDMI_4":
		return "tx 1f:82:40:00", nil
	case "POWER_ON":
		return fmt.Sprintf("on %s", TV_ID), nil
	case "POWER_OFF":
		return fmt.Sprintf("standby %s", TV_ID), nil
	case "SCAN":
		return fmt.Sprintf("scan"), nil
	default:
...
func runCecClient() {
	clientName := getEnv("OSD_NAME", "Kodi")
	fmt.Printf("Setting OSD name to %s\n", clientName)
	c := exec.Command("cec-client", "-d", "1", "-o", clientName)
